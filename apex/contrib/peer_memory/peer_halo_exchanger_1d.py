    ...
    def __call__(self, y, H_split=True, explicit_nhwc=False, numSM=1, diagnostics=False):
        ...
        if H_split:
            if explicit_nhwc:
                ...
                low_halo_shape = [1,1,1,4*low_out_halo.numel()]
                low_tx = self.peer_pool.allocate_peer_tensors(low_halo_shape, low_out_halo.dtype, False, True)
                low_inp_halo = y[:,:self.half_halo,:,:]
                high_out_halo = y[:,H:H+self.half_halo,:,:]
                high_halo_shape = [1,1,1,4*high_out_halo.numel()]
                high_tx = self.peer_pool.allocate_peer_tensors(high_halo_shape, high_out_halo.dtype, False, True)
